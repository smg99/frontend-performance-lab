...
export const backgroundProcessingManifest: Recipe = {
  ...
  decisionMatrix: [
    {
      scenario:
        'You need to filter a massive dataset (e.g. 50,000 objects) without freezing the UI.',
      recommendedApproach: 'Web Workers',
      alternatives: ['Time Slicing (setTimeout)'],
      tradeoffs:
        'Requires transferring data via postMessage, which has serialization costs. You cannot access the DOM.',
    ...
    {
      scenario:
        'You need to render a complex component tree progressively, but you cannot use a Worker because it relies on the DOM.',
      recommendedApproach: 'Time Slicing (Yielding to Main Thread)',
      alternatives: ['Web Workers'],
      tradeoffs:
        'The overall processing time will be slightly longer due to context switching, but the UI remains responsive.',
  ...
  implementationSteps: [
    {
      ...
      description:
        'Extract the heavy calculation into a pure function that does not rely on closures, DOM APIs, or global state.'
    },
    {
      title: '2. Spin up the Worker',
      description:
        'Create a separate `.js` file for the worker and instantiate it using `new Worker()`. Consider using libraries like `Comlink` for RPC.'
    },
    {
      title: '3. Handle Asynchrony',
      description:
        'Update the UI component to display a loading state while waiting for the Worker to `postMessage` the result back.'
    }
  ],
...
const handleSearch = async (query) => {
  ...
  worker.onmessage = (e) => {
    ...
    explanation:
      'In the before code, the synchronous execution prevents the browser from ever rendering the loading spinner. In the after code, the main thread is instantly freed, the spinner renders, and the calculation happens invisibly in the background.'
  },
  ...
  interviewQuestions: [
    {
      question:
        'What is the "Total Blocking Time" (TBT) metric, and how do Web Workers help improve it?',
      answer:
