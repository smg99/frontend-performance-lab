  ...
  decisionMatrix: [
    {
      ...
      tradeoffs:
        'Breaks native browser "Find in Page" (Ctrl+F) because unrendered rows do not exist in the DOM.',
    ...
    },
    {
      ...
      tradeoffs:
        'UX friction; users have to explicitly click to see more data. However, native browser search works perfectly for the current page.',
  ...
  implementationSteps: [
    {
      ...
      description:
        'Ensure each row has a predictable height. This is critical for the virtualization engine to calculate scroll positions instantly without measuring the DOM.'
    },
    {
      title: '2. Implement the Scroller Component',
      description:
        'Replace your `<table>` or `<ul>` with a VirtualScroller. Pass the total item count and the row height.'
    },
    {
      title: '3. Render the Visible Slice',
      description:
        'The Scroller will provide you with the `index` and `style` props. Apply the `style` (which contains absolute positioning) directly to the row.'
    }
  ],
...
const DataTable = ({ data }) => {
  ...
  return (
    ...
    explanation:
      'The before code blocks the main thread completely. The after code uses virtualization to absolute position only the items currently inside the 800px viewport.'
  },
  ...
  interviewQuestions: [
    {
      question:
        'What is DOM virtualization (windowing), and how does it improve performance for large lists?',
      answer:
        'Virtualization restricts DOM rendering to only the items currently visible in the viewport, plus a small buffer. Instead of rendering 10,000 DOM nodes, it renders ~20. It achieves this by listening to scroll events and absolutely positioning the items based on their index and fixed height, drastically reducing memory usage and main thread blocking.'
    }
  ],
