...
export const lazyLoadImageMisses: ASTRule = {
  ...
  description: 'Images missing `loading="lazy"` can delay page rendering, especially when many images are present.',
  severity: 'Warning',
  ...
  confidence: { score: 85, reasoning: 'Detects JSX img or HTML img elements without loading attribute.', falsePositiveRisk: 'Low' },
  visitor: (ast: any, context: AnalyzerContext) => {
    ...
    if (imgCount >= 5) {
      for (const p of imgElements) {
        const hasLoading = p.node.attributes.some((attr: any) => attr.name && attr.name.name === 'loading');
        if (!hasLoading) {
