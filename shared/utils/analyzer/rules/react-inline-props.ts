...
export const reactInlineProps: ASTRule = {
  ...
  impact: 'Inline objects, arrays, or functions create a new reference on every render, defeating React.memo() equality checks and forcing child components to re-render.',
  ...
  explanation: {
    whatHappened: 'An inline object `{}` or inline function `() => {}` was passed as a prop to a custom component.',
    whyBrowserBehavesThisWay: 'React checks prop equality by reference (===). Inline structures create a new reference in memory every time the parent renders, making the child think its props changed.',
    pipelineInvolved: ['DOM']
  },
  autoFix: {
    badCode: '<MyComponent data={{ key: "value" }} onClick={() => doSomething()} />',
    recommendedCode: 'const data = useMemo(() => ({ key: "value" }), []);\\nconst onClick = useCallback(() => doSomething(), []);\\n<MyComponent data={data} onClick={onClick} />',
    whyFaster: 'Memoization preserves the memory reference between renders, allowing React to safely skip re-rendering the child component.'
  },
  confidence: {
    score: 95,
    reasoning: 'AST definitively identifies inline objects, arrays, and functions inside JSX attributes.',
    limitations: 'Cannot detect if the child component is actually wrapped in React.memo (unless analyzing cross-file).',
