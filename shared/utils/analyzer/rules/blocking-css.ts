...
export const blockingCss: ASTRule = {
  ...
  description: 'Synchronous CSS stylesheets block the browser from rendering the page until the CSS is downloaded and parsed.',
  severity: 'Warning',
  ...
  confidence: { score: 80, reasoning: 'Detects link elements with rel="stylesheet" lacking media="print". Cannot determine if CSS is genuinely critical.', falsePositiveRisk: 'Medium' },
      ...
      JSXOpeningElement(path: any) {
        ...
        if (nameNode && nameNode.name === 'link') {
          ...
          for (const attr of path.node.attributes) {
            if (attr.name && attr.name.name === 'rel' && attr.value && attr.value.value === 'stylesheet') {
              isStylesheet = true;
            }
            if (attr.name && attr.name.name === 'media' && attr.value && attr.value.value === 'print') {
              isNonBlocking = true;
            }
