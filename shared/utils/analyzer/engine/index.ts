import type { AnalyzerContext, ASTRule, Issue, ReviewReport, OptimizationChecklist, PerformanceMetrics } from '../../../schemas/analyzer'
  ...
  public analyze(contexts: AnalyzerContext[]): ReviewReport {
    ...
    for (const ctx of contexts) {
      ...
      for (const rule of this.rules) {
        if (rule.supportedLanguages.includes(ctx.language) && rule.frameworks.includes(ctx.framework)) {
    ...
    const grade = finalScore >= 95 ? 'A+' : finalScore >= 90 ? 'A' : finalScore >= 80 ? 'B' : finalScore >= 70 ? 'C' : finalScore >= 60 ? 'D' : 'F'

    const checklist: OptimizationChecklist[] = Array.from(new Set(allIssues.map(i => i.fix))).map(fix => ({
