...
2. **MCP Integration:** `mcp/server.ts` is the bridge to the outside world. It exposes our knowledge graph to AI agents like Claude, Cursor, and Windsurf via stdio. It also provides the `performance_audit` tool, which currently returns mock data but will eventually connect to the AST analyzer.
