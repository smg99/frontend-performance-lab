...
import { searchExperiments } from '../shared/utils/search/index.js'
...
const EXPERIMENT_SECTIONS = z.enum([
  'summary',
  'examples',
  'interview',
  'references',
...
server.resource(
  ...
  async (uri, { id }) => {
    ...
    return {
      contents: [{
        uri: uri.href,
        text: JSON.stringify({ checklist: ["Virtualize long lists", "Use Web Workers for heavy JS", "Avoid Layout Thrashing"] }, null, 2)
      }]
    }
  }
)
...
server.tool(
  ...
  "Get the full manifest or a specific section of a performance experiment. Examples of sections: 'examples', 'interview', 'summary'.",
  {
    ...
    section: EXPERIMENT_SECTIONS.optional().describe("Optional: Only return a specific section to save context window tokens.")
  },
  async ({ id, section }) => {
    const exp = getExperimentById(id)
    if (!exp) {
    ...
    if (section) {
      ...
      switch(section) {
        ...
        case 'summary':
        default:
          filteredData = exp.sections.filter(s => s.type === 'concept' || s.type === 'recommendation')
          break
      }
      return {
...
server.tool(
  ...
  "Search the entire performance knowledge base. Use this to find solutions to specific problems (e.g. 'layout thrashing', 'memory leak') or to lookup browser APIs.",
  {
    ...
    type: z.enum(['experiment', 'recipe', 'browser-api']).optional().describe("Filter by content type"),
    difficulty: z.enum(['Beginner', 'Intermediate', 'Advanced']).optional().describe("Filter by difficulty level"),
    tags: z.array(z.string()).optional().describe("Filter by specific tags (e.g., ['vue', 'rendering'])"),
    browserAPI: z.string().optional().describe("Filter by browser API (e.g., 'requestAnimationFrame')"),
    limit: z.number().optional().describe("Maximum number of results to return (default: all)")
  },
  async (filters) => {
    const results = searchExperiments(filters)
    return {
...
server.tool(
  ...
  async () => {
    ...
    const testQueries = ['virtualization', 'rendering', 'memory', 'worker', 'requestAnimationFrame', 'CLS']
    const Search = {
      TestQueries: testQueries.map(q => {
        const res = searchExperiments({ query: q })
        return {
          query: q,
          resultCount: res.length,
          topResult: res[0]?.id || null
        }
      })
    }
    ...
    if (Validation.Issues.length === 0) Validation.Issues.push("[PASS] All manifests passed strict schema validation.")

    // Stats
    const totalContentSize = JSON.stringify(all).length
    const totalRefs = all.reduce((acc, exp) => acc + exp.references.length, 0)
    const totalExamples = all.reduce((acc, exp) => acc + exp.sections.filter(s => s.type === 'example').length, 0)
    const totalInterviews = all.reduce((acc, exp) => acc + exp.sections.filter(s => s.type === 'interview').length, 0)

    const Statistics = {
      TotalContentSizeBytes: totalContentSize,
      AverageManifestSizeBytes: totalContentSize / (all.length || 1),
      AverageSectionCount: all.reduce((acc, exp) => acc + exp.sections.length, 0) / (all.length || 1),
    ...
    const report = {
      ...
      Resources: {
        ...
        ResourceURIs: ["performance://experiments/{id}", "performance://checklists/{id}"]
      },
      Prompts: {
        TotalPromptCount: 1,
        List: [{ name: "review_performance", description: "Ask AI to review code for performance." }]
      },
      Experiments,
      BrowserAPIs: { Count: 0, Names: [] },
      ...
      Overall: {
        HealthScore: Validation.OverallStatus === 'PASS' ? 100 : Validation.OverallStatus === 'WARNING' ? 85 : 50,
        CriticalIssues: Validation.Issues.filter(i => i.includes('[ERROR]')),
        Warnings: Validation.Issues.filter(i => i.includes('[WARNING]')),
        Recommendations: [
          "Implement Browser API registry",
