...
**Solution:** Ensure you are running `npm run generate` instead of `npm run build`, and that `ssr: false` is configured in `nuxt.config.ts` if targeting pure static hosting. *(Note: The MCP Hub UI is currently designed for Nuxt dynamic routing).*
