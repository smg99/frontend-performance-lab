<template>
  ...
  >
    ...
    <!-- Sidebar: File Tree -->
    <div
      class="flex-shrink-0 border-r border-border-subtle-subtle bg-background-surface flex flex-col relative"
      :style="{ width: leftPaneWidth + 'px' }"
    >
      <div
        class="p-4 border-b border-border-subtle-subtle flex justify-between items-center bg-background-surface sticky top-0 z-10"
      >
        <h3 class="font-semibold text-foreground-primary text-sm uppercase tracking-wider">
          Explorer
        </h3>
        <button
          class="text-foreground-muted hover:text-primary transition-colors p-1"
          title="New File"
          @click="addFile"
        >
      ...
      <div class="flex-1 overflow-y-auto p-2 space-y-1">
        ...
        <div
          v-if="files.length === 0"
          class="p-4 text-center text-text-muted text-sm border border-dashed border-border-subtle-subtle rounded m-2"
        >
      ...
      <div
        class="p-4 border-t border-border-subtle-subtle space-y-2 bg-background-surface sticky bottom-0 z-10"
      >
        <Button
          class="w-full"
    ...
    <!-- Left Resizer -->
    <div
      class="w-1 hover:bg-primary cursor-col-resize z-20 flex-shrink-0 transition-colors bg-border-subtle"
      ...
      <div class="flex-1 relative bg-background-base">
        <div v-if="files.length > 0 && activeFile" class="absolute inset-0">
          <CodeEditor
            ...
            v-model:language="activeFile.language"
            @analyze="handleAnalyzeShortcut"
          />
        </div>
        <div
          v-else
          class="absolute inset-0 flex flex-col items-center justify-center text-foreground-muted bg-background-surface/30"
        >
          <div class="max-w-md w-full text-center space-y-6 p-8">
            <div
              class="w-16 h-16 rounded-2xl bg-background-base border border-border-subtle-strong flex items-center justify-center mx-auto shadow-sm"
            >
              <CodeIcon class="w-8 h-8 text-primary opacity-80" />
            </div>
            <div>
              <h3 class="text-xl font-bold text-foreground-primary tracking-tight mb-2">
                Analyzer Workspace
              </h3>
              <p class="text-sm leading-relaxed">
                Paste your code, drag a folder, or load a sample snippet to see the AST engine
            ...
            <div class="grid grid-cols-2 gap-4 pt-4">
              <button
                class="flex flex-col items-start p-4 rounded-xl border border-border-subtle-subtle bg-background-base hover:border-emerald-500/50 hover:bg-emerald-500/5 hover:-translate-y-[1px] transition-all duration-220 text-left group"
              ...
              >
                ...
                <span class="text-xs text-foreground-muted line-clamp-2"
                  >Layout thrashing & render loops</span
                >
              </button>
              ...
              >
                ...
                <span class="text-xs text-foreground-muted line-clamp-2"
                  >Reactivity loss & excessive watchers</span
                >
              </button>
            </div>
          </div>
    ...
    <!-- Right Pane: Review -->
    <div
      class="flex-shrink-0 border-l border-border-subtle-subtle bg-background-surface relative flex flex-col"
    ...
    >
      ...
      >
        <div
          class="bg-background-base border border-border-subtle-subtle p-6 rounded-xl shadow-xl flex flex-col items-center max-w-[80%] text-center"
        >
          <div
            class="w-8 h-8 border-4 border-primary border-t-transparent rounded-full animate-spin mb-4"
          />
          <span class="text-foreground-primary font-bold mb-2">Analyzing...</span>
