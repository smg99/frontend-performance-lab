<template>
  <div
    class="px-6 py-4 bg-background-base border-b border-border-subtle-subtle flex items-center justify-between text-xs font-mono overflow-x-auto"
  >
    <div v-for="(stage, idx) in stages" :key="stage.id" class="flex items-center flex-shrink-0">
      ...
      <div
        ...
        class="w-6 h-[2px] mx-2 transition-all duration-500 relative overflow-hidden"
        :class="getLineClass(idx)"
      >
        <div
          v-if="isCurrent(stage.id)"
          class="absolute inset-0 bg-primary/50 translate-x-[-100%] animate-[slideRight_1s_infinite]"
        />
      </div>
    </div>
  </div>
...
const getStateClass = (stageId: string) => {
  ...
  return 'bg-background-surface border-border-subtle-subtle text-text-muted opacity-50' // Pending
}
