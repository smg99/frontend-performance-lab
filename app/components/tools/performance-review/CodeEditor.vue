  ...
  <div class="h-full flex flex-col bg-surface border-r border-border">
    <div class="p-4 border-b border-border flex justify-between items-center bg-card">
      ...
      <div class="flex gap-2">
        <select v-model="localLang" class="bg-surface text-text-secondary border border-border rounded px-2 py-1 text-sm">
          <option value="vue">Vue</option>
          <option value="js">JavaScript</option>
          <option value="ts">TypeScript</option>
          <option value="jsx">React (JSX)</option>
        </select>
        <button @click="$emit('analyze', { code: localCode, language: localLang })" class="bg-primary text-white px-4 py-1 rounded text-sm hover:bg-primary/90 transition-colors shadow-subtle">
          Analyze Code
        </button>
      </div>
    </div>
