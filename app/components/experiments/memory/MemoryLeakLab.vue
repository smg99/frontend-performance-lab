<template>
  <div class="bg-card border border-border p-6 rounded-xl shadow-subtle flex flex-col gap-6">
    <div class="flex items-center gap-4 bg-surface p-4 rounded-lg border border-border">
      <div class="flex-1">
        <h4 class="text-sm font-semibold text-text-primary mb-1">Detached DOM Leak</h4>
        <p class="text-xs text-text-secondary">If you remove a DOM node from the document, but keep a JavaScript reference to it, the browser cannot garbage collect it. This is a "Detached DOM" memory leak.</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="bg-surface border border-border p-4 rounded-lg flex flex-col gap-3">
          <h5 class="font-bold text-sm text-text-primary">Leak Mechanism</h5>
          <button 
          ...
          <div v-pre class="bg-background p-3 rounded font-mono text-xs border border-border mt-2">
        ...
        <div class="bg-surface border border-border p-4 rounded-lg flex flex-col gap-3">
          <h5 class="font-bold text-sm text-text-primary">Fix Mechanism</h5>
          <button 
      ...
      <div class="bg-surface border border-border p-4 rounded-lg flex flex-col h-[350px]">
        <div class="flex justify-between items-center mb-4">
          <h5 class="font-bold text-sm text-text-primary">Simulated Heap Growth</h5>
          <div class="text-xs font-mono font-bold" :class="nodeCount > 50000 ? 'text-danger' : 'text-text-secondary'">
            Nodes: {{ (nodeCount).toLocaleString() }}
          </div>
        </div>
        
        <div class="flex-1 bg-background border border-border rounded relative flex items-end p-2 gap-1 overflow-hidden">
          <div 
          ...
          />
          <div v-if="history.length === 0" class="absolute inset-0 flex items-center justify-center text-text-secondary text-xs italic">
            Waiting for allocations...
          </div>
        </div>
        
        <div class="mt-4 text-[10px] text-text-secondary text-center">
          In a real application, you can view this exact graph in Chrome DevTools -> Memory -> Allocation timeline.
        </div>
      </div>
