<template>
  <div class="bg-card border border-border p-6 rounded-xl shadow-subtle flex flex-col gap-6">
    <div class="flex items-center gap-4 bg-surface p-4 rounded-lg border border-border">
      <div class="flex-1">
        <h4 class="text-sm font-semibold text-text-primary mb-1">Heap & Mark-and-Sweep</h4>
        <p class="text-xs text-text-secondary">Watch the Garbage Collector traverse from the Window (root), mark reachable objects, and sweep detached ones.</p>
      </div>
      <div class="flex gap-2">
        <button 
          :disabled="isGCActive" 
          class="px-3 py-1.5 bg-background text-text-primary border border-border rounded text-sm hover:bg-surface transition-colors disabled:opacity-50"
    ...
    <!-- Heap Area -->
    <div class="bg-background border border-border rounded-lg h-[300px] relative overflow-hidden p-6 flex items-center justify-center">
      
      <!-- Window Root -->
      <div class="absolute left-6 top-1/2 -translate-y-1/2 flex flex-col items-center z-20">
        <div class="w-16 h-16 bg-primary rounded-full shadow-lg flex items-center justify-center text-white font-bold border-4 border-primary-foreground/30 relative" :class="{ 'ring-4 ring-success animate-pulse': isMarking }">
          Window
        </div>
        <div class="text-[10px] text-text-secondary mt-2 font-mono">GC Root</div>
      </div>
      ...
      <div class="ml-24 w-full h-full relative">
        <transition-group name="obj-pop">
          <div 
            ...
            :class="[
              obj.isMarked ? 'bg-success/20 text-success border-success' : 
              obj.isSwept ? 'bg-danger/20 text-danger border-danger scale-90 opacity-0' : 
        ...
        <svg class="absolute inset-0 w-full h-full pointer-events-none z-10" style="overflow: visible;">
          ...
          <line 
            ...
            x1="-20" y1="50%" 
            :x2="`${obj.x}%`" :y2="`${obj.y + (obj.isMarked ? 1 : 0)}%`" 
            class="transition-all duration-500 stroke-2"
            :class="obj.isMarked ? 'stroke-success' : obj.isSwept ? 'stroke-transparent' : 'stroke-border'"
          />
        </svg>
      </div>

      <!-- GC Overlay Text -->
      <div v-if="gcPhase" class="absolute inset-0 flex items-center justify-center pointer-events-none z-30 bg-background/50 backdrop-blur-sm">
        <h2 class="text-4xl font-bold font-mono tracking-widest uppercase" :class="gcPhase === 'MARKING' ? 'text-success' : 'text-danger'">
          {{ gcPhase }}
        </h2>
      </div>
    </div>
