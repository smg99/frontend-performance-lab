<template>
  ...
  >
    ...
    >
      <div class="flex-1">
        ...
        <p class="text-xs text-foreground-muted">
          Click any state node (Ref/Reactive) below to simulate Vue's dependency triggering process.
        </p>
      </div>
      <div class="flex gap-2">
        <button
          class="px-3 py-1.5 bg-background border border-border-subtle rounded text-sm transition-colors text-foreground-muted hover:text-foreground-primary"
          @click="resetZoom"
        >
          Reset View
        </button>
    ...
    <div
      class="w-full h-[500px] bg-background border border-border-subtle rounded-xl overflow-hidden relative select-none cursor-grab active:cursor-grabbing"
      @mousedown="startPan"
      @mousemove="doPan"
      @mouseup="endPan"
      @mouseleave="endPan"
      ...
      <svg class="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        ...
        <g :transform="`translate(${panX}, ${panY}) scale(${zoom})`">
          ...
          <circle
            ...
            fill="#60A5FA"
          >
            <animateMotion :path="calculatePath(edge)" dur="0.6s" repeatCount="1" />
          </circle>
