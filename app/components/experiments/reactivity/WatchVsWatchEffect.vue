  ...
  <div class="bg-card border border-border p-6 rounded-xl shadow-subtle flex flex-col gap-6">
    <div class="flex items-center gap-4 bg-surface p-4 rounded-lg border border-border">
      <div class="flex flex-col">
        <label class="text-xs text-text-secondary font-semibold uppercase mb-1">Search Query</label>
        <input 
          v-model="searchQuery" 
          type="text" 
          placeholder="Type to search..."
          class="bg-background border border-border text-sm rounded px-3 py-1.5 outline-none focus:border-primary text-text-primary w-64"
        >
      </div>
      <div class="flex flex-col">
        <label class="text-xs text-text-secondary font-semibold uppercase mb-1">Filter Type</label>
        <select v-model="filterType" class="bg-background border border-border text-sm rounded px-3 py-1.5 outline-none focus:border-primary text-text-primary">
      ...
      <div class="flex-1"/>
      <button class="px-4 py-2 bg-surface hover:bg-border/50 border border-border text-text-primary rounded-lg font-medium transition-colors shadow-subtle text-sm flex items-center gap-2" @click="triggerUnrelated">
        Trigger Unrelated State
        <span class="bg-primary/20 text-primary px-2 py-0.5 rounded text-xs font-bold">{{ unrelatedState }}</span>
      </button>
    </div>
      ...
      <div class="space-y-4">
        <div class="flex justify-between items-center border-b border-border pb-2">
          <h4 class="text-lg font-bold text-primary flex items-center gap-2">
            watch() 
            <span class="text-xs bg-surface border border-border px-2 py-0.5 rounded font-mono font-normal text-text-secondary">Explicit dependencies</span>
          </h4>
        </div>
        
        <div class="bg-surface p-4 rounded-lg border border-border space-y-3 font-mono text-sm">
          ...
          <div class="flex justify-between items-center">
            <span class="text-text-secondary">Cleanup Executions:</span>
            <span ref="watchCleanupCountNode" class="font-bold text-warning">0</span>
          </div>
          <div class="mt-4 pt-4 border-t border-border">
            <div class="text-text-secondary mb-1">Latest Event:</div>
            <div ref="watchLogNode" class="text-xs text-text-primary break-all">Waiting...</div>
          </div>
        </div>
      </div>
      ...
      <div class="space-y-4">
        <div class="flex justify-between items-center border-b border-border pb-2">
          <h4 class="text-lg font-bold text-success flex items-center gap-2">
            watchEffect() 
            <span class="text-xs bg-surface border border-border px-2 py-0.5 rounded font-mono font-normal text-text-secondary">Automatic tracking</span>
          </h4>
        </div>
        ...
        <div class="bg-surface p-4 rounded-lg border border-border space-y-3 font-mono text-sm">
          ...
          <div class="mt-4 pt-4 border-t border-border">
            <div class="text-text-secondary mb-1">Latest Event:</div>
            <div ref="effectLogNode" class="text-xs text-text-primary break-all">Waiting...</div>
          </div>
        </div>
      </div>
    </div>
...
const updateNodes = () => {
  if (import.meta.client) {
    ...
    if (watchCleanupCountNode.value) watchCleanupCountNode.value.textContent = watchCleanups.toString()
    if (effectExecCountNode.value) effectExecCountNode.value.textContent = effectCount.toString()
    if (effectCleanupCountNode.value) effectCleanupCountNode.value.textContent = effectCleanups.toString()
  }
}
...
watchEffect((onCleanup) => {
  ...
  logEffect(`Triggered by tracked deps. Query: "${query}", Filter: "${filter}", Unrelated: ${unrelated}`)
