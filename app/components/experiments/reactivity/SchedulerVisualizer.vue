<template>
  ...
  >
    ...
    >
      <div class="flex-1">
        <h4 class="text-sm font-semibold text-foreground-primary mb-1">
          Async Scheduler & Batching
        </h4>
        <p class="text-xs text-foreground-muted">
          Simulates Vue's nextTick scheduler. Rapid mutations are deduplicated into a single flush
          cycle.
        </p>
      </div>
      <button
        :disabled="isFlushing"
        class="px-4 py-2 bg-primary text-primary-foreground rounded-lg font-medium transition-colors shadow-subtle text-sm disabled:opacity-50"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
      <!-- Queue Status -->
      <div>
        <h5 class="text-sm font-bold text-foreground-muted mb-3">Job Queue (Microtasks)</h5>
        <div
          class="bg-background-surface border border-border-subtle rounded-lg p-4 space-y-2 h-[200px] overflow-hidden flex flex-col justify-end"
        >
          ...
          <div
            v-if="queue.length === 0"
            class="text-foreground-muted text-xs italic text-center py-2"
          >
            Queue is empty
          </div>
        </div>
      ...
      <!-- Render/Flush status -->
      <div
        class="flex flex-col items-center justify-center bg-background-surface border border-border-subtle rounded-lg p-6 relative overflow-hidden"
      >
        <h5 class="text-sm font-bold text-foreground-muted mb-6 absolute top-4 left-4">
          Scheduler Flush
        </h5>
        ...
        <div
          ...
          :class="
            isFlushing
              ? 'bg-success/20 border-success shadow-[0_0_30px_rgba(34,197,94,0.4)] scale-110'
              : 'bg-background border-border-subtle scale-100'
          "
        >
          <div class="flex flex-col">
            <span
              class="font-bold text-lg"
              :class="isFlushing ? 'text-success' : 'text-foreground-primary'"
            >
        ...
        <div class="absolute bottom-4 right-4 text-xs font-mono text-foreground-muted">
