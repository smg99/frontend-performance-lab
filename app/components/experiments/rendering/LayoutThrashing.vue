<template>
  ...
  >
    ...
    >
      <div class="flex-1">
        <h4 class="text-sm font-semibold text-foreground-primary mb-1">
          Layout Thrashing vs Batching
        </h4>
        <p class="text-xs text-foreground-muted">
          Compare a naïve read/write loop vs batched DOM operations across 500 elements.
        </p>
      </div>
    ...
    <div v-if="lastRun" class="flex gap-4">
      <div
        class="bg-background-surface border border-border-subtle rounded px-4 py-3 flex flex-col items-center flex-1"
      >
        <span class="text-xs text-foreground-muted uppercase mb-1">Execution Time</span>
        <span
          class="text-xl font-bold"
          :class="lastRun.type === 'thrash' ? 'text-danger' : 'text-success'"
        >
      ...
      <div
        class="bg-background-surface border border-border-subtle rounded px-4 py-3 flex flex-col items-center flex-1"
      >
        <span class="text-xs text-foreground-muted uppercase mb-1">FPS Drop</span>
        <span
          class="text-xl font-bold"
          :class="lastRun.type === 'thrash' ? 'text-danger' : 'text-success'"
        >
      ...
      <div
        class="bg-background-surface border border-border-subtle rounded px-4 py-3 flex flex-col items-center flex-1"
      >
        <span class="text-xs text-foreground-muted uppercase mb-1">Recalculate Style Count</span>
        <span
          class="text-xl font-bold"
          :class="lastRun.type === 'thrash' ? 'text-danger' : 'text-success'"
        >
    ...
    <div class="bg-background border border-border-subtle p-4 rounded-lg">
      <div class="text-xs text-foreground-muted mb-2 flex justify-between">
