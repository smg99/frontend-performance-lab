<template>
  <div class="bg-card border border-border p-6 rounded-xl shadow-subtle flex flex-col gap-6">
    <div class="flex items-center gap-4 bg-surface p-4 rounded-lg border border-border">
      <div class="flex-1">
        <h4 class="text-sm font-semibold text-text-primary mb-1">Layout vs Composite Animations</h4>
        <p class="text-xs text-text-secondary">Comparing the performance cost of animating layout properties vs composite properties on 200 elements.</p>
      </div>
      <div class="flex gap-2">
        <button 
          class="px-3 py-1.5 rounded text-sm font-medium transition-colors border"
          :class="isAnimatingLayout ? 'bg-danger/20 text-danger border-danger/50' : 'bg-surface text-text-secondary hover:text-text-primary border-border'"
        ...
        <button 
          class="px-3 py-1.5 rounded text-sm font-medium transition-colors border"
          :class="isAnimatingComposite ? 'bg-success/20 text-success border-success/50' : 'bg-surface text-text-secondary hover:text-text-primary border-border'"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="bg-surface border border-border rounded-lg p-4 flex flex-col relative overflow-hidden h-[300px]">
        <div class="flex justify-between items-center mb-4 z-10 relative bg-surface/90 pb-2 border-b border-border">
          <div>
            <h5 class="text-sm font-bold text-danger">Layout Animation</h5>
            <div class="text-[10px] text-text-secondary font-mono mt-1">el.style.left = px</div>
          </div>
          <div class="text-right">
            <div class="text-[10px] text-text-secondary uppercase">FPS</div>
        ...
        <div v-if="isAnimatingLayout" class="absolute bottom-2 right-2 bg-background/90 border border-border p-2 rounded text-[10px] font-mono shadow-lg z-20">
      ...
      <div class="bg-surface border border-border rounded-lg p-4 flex flex-col relative overflow-hidden h-[300px]">
        <div class="flex justify-between items-center mb-4 z-10 relative bg-surface/90 pb-2 border-b border-border">
          <div>
            ...
            <div class="text-[10px] text-text-secondary font-mono mt-1">el.style.transform = translate3d</div>
          </div>
          <div class="text-right">
            <div class="text-[10px] text-text-secondary uppercase">FPS</div>
        ...
        <!-- Live Metrics Overlay -->
        <div v-if="isAnimatingComposite" class="absolute bottom-2 right-2 bg-background/90 border border-border p-2 rounded text-[10px] font-mono shadow-lg z-20">
          <div class="text-text-secondary">Layouts: 0/sec</div>
          <div class="text-text-secondary">Paints: 0/sec</div>
...
const updateLayout = () => {
  ...
  while(performance.now() - start < 8) { /* block */ }
