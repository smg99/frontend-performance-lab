<template>
  <div class="bg-card border border-border p-6 rounded-xl shadow-subtle flex flex-col gap-6">
    <div class="flex items-center gap-4 bg-surface p-4 rounded-lg border border-border">
      <div class="flex-1">
        <h4 class="text-sm font-semibold text-text-primary mb-1">Paint Flashing Simulator</h4>
        <p class="text-xs text-text-secondary">Simulates Chrome DevTools' Paint Flashing. Watch how changing a layout property repaints the parent, but a transform does not.</p>
      </div>
      <div class="flex gap-2">
        <label class="flex items-center gap-2 text-sm text-text-secondary cursor-pointer">
          <input v-model="flashingEnabled" type="checkbox" class="rounded border-border bg-background text-success focus:ring-success">
          Enable Paint Flashing
        </label>
      </div>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-6" :class="{ 'paint-flashing-active': flashingEnabled }">
      
      <!-- Layout Example -->
      <div class="bg-surface border border-border rounded-lg p-6 flex flex-col relative overflow-hidden group">
        <h5 class="text-sm font-bold text-text-secondary mb-4 z-20">Triggers Layout & Paint</h5>
        ...
        <div class="flash-overlay absolute inset-0 bg-success/30 z-10 pointer-events-none opacity-0 transition-opacity duration-[50ms]" :class="{ 'opacity-100': isFlashingLayout }"/>
      </div>

      <!-- Composite Example -->
      <div class="bg-surface border border-border rounded-lg p-6 flex flex-col relative overflow-hidden group">
        <h5 class="text-sm font-bold text-text-secondary mb-4 z-20">Triggers Composite Only</h5>
        <div class="flex-1 min-h-[150px] relative z-20">
          <div 
        ...
        <div class="flash-overlay absolute inset-0 bg-success/30 z-10 pointer-events-none opacity-0 transition-opacity duration-[50ms]" :class="{ 'opacity-100': isFlashingComposite }"/>
      </div>
