<template>
  ...
  >
    ...
    >
      <div class="flex-1">
        ...
        <p class="text-xs text-foreground-muted">
          Measures when the largest text block or image element becomes visible within the viewport.
          Good: &lt; 2.5s.
        </p>
      </div>
    ...
    <!-- Timeline View -->
    <div
      class="bg-background-surface border border-border-subtle p-4 rounded-lg flex flex-col relative overflow-hidden h-[200px]"
    >
      <!-- Axis -->
      <div
        class="flex text-[10px] text-foreground-muted font-mono border-b border-border-subtle pb-2 mb-4"
      >
      ...
      <div class="relative flex-1">
        ...
        <div class="flex items-center mb-3">
          <div class="w-24 text-[10px] uppercase font-bold text-foreground-muted">HTML (TTFB)</div>
          <div
            class="h-4 bg-blue-500 rounded text-[10px] font-bold text-white flex items-center px-2 transition-all duration-[800ms] overflow-hidden whitespace-nowrap"
            :style="{ width: phases.ttfb + '%' }"
          >
            Network Request
          </div>
        </div>
        ...
        <!-- Render Blocking CSS -->
        <div class="flex items-center mb-3">
          <div class="w-24 text-[10px] uppercase font-bold text-foreground-muted">CSS / Fonts</div>
          <!-- Starts after TTFB -->
          <div class="h-4 bg-transparent" :style="{ width: phases.ttfb + '%' }" />
          <div
            class="h-4 bg-purple-500 rounded text-[10px] font-bold text-white flex items-center px-2 transition-all duration-[800ms] overflow-hidden whitespace-nowrap"
            :style="{ width: phases.css + '%' }"
          >
            Render Blocking
          </div>
        </div>
        ...
        <div class="flex items-center">
          <div class="w-24 text-[10px] uppercase font-bold text-foreground-muted">Hero Image</div>
          ...
          <div
            ...
            :class="
              lcpScore === 'GOOD' ? 'bg-success' : lcpScore === 'POOR' ? 'bg-danger' : 'bg-warning'
            "
      ...
      <div
        v-if="lcpResult"
        class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-background-base border border-border-subtle p-4 rounded-xl shadow-xl flex flex-col items-center gap-2 z-20"
      >
        <h2
          class="text-3xl font-bold font-mono"
          :class="lcpScore === 'GOOD' ? 'text-success' : 'text-danger'"
        >
          {{ lcpResult.toFixed(1) }}s
        </h2>
        <div class="text-xs uppercase font-bold tracking-widest text-foreground-muted">
          {{ lcpScore }}
        </div>
      </div>
    </div>
