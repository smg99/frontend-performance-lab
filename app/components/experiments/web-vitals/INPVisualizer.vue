<template>
  <div class="bg-card border border-border p-6 rounded-xl shadow-subtle flex flex-col gap-6">
    <div class="flex items-center gap-4 bg-surface p-4 rounded-lg border border-border">
      <div class="flex-1">
        <h4 class="text-sm font-semibold text-text-primary mb-1">Interaction to Next Paint (INP)</h4>
        <p class="text-xs text-text-secondary">Measures the latency of every tap, click, or keyboard interaction. Good: &lt; 200ms. Poor: &gt; 500ms.</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="bg-surface border border-border rounded-lg p-6 flex flex-col gap-4">
        ...
        <div class="h-10 border border-border rounded bg-background flex items-center justify-center font-mono text-sm text-text-secondary" :class="{ 'bg-primary/10 text-primary font-bold': clicks > 0 }">
          Clicks Processed: {{ clicks }}
        </div>
      </div>

      <!-- INP Timeline -->
      <div class="bg-surface border border-border rounded-lg p-4 flex flex-col justify-center relative overflow-hidden h-[250px]">
        <h5 class="text-xs font-bold text-text-secondary uppercase tracking-widest mb-4">Latest Interaction Latency</h5>
        
        <!-- Graph -->
        <div class="flex-1 flex flex-col gap-2 font-mono text-[10px]">
          
          <div class="flex items-center gap-2">
            <div class="w-16 text-right text-text-secondary">Input Delay</div>
            <div class="h-4 bg-blue-500 rounded transition-all duration-300" :style="{ width: `${Math.min(inputDelay / 5, 100)}%` }"/>
            <div class="text-blue-400 font-bold">{{ inputDelay.toFixed(1) }}ms</div>
          </div>
          
          <div class="flex items-center gap-2">
            <div class="w-16 text-right text-text-secondary">Processing</div>
            <div class="h-4 bg-purple-500 rounded transition-all duration-300" :style="{ width: `${Math.min(processingTime / 5, 100)}%` }"/>
            <div class="text-purple-400 font-bold">{{ processingTime.toFixed(1) }}ms</div>
          </div>
          
          <div class="flex items-center gap-2">
            <div class="w-16 text-right text-text-secondary">Presentation</div>
            <div class="h-4 bg-success rounded transition-all duration-300" :style="{ width: `${Math.min(presentationDelay / 5, 100)}%` }"/>
        ...
        <div class="border-t border-border pt-4 mt-4 flex justify-between items-end">
          <div>
            <div class="text-xs text-text-secondary uppercase font-bold">Total INP</div>
            <div class="text-[10px]" :class="inpScore === 'GOOD' ? 'text-success' : inpScore === 'POOR' ? 'text-danger' : 'text-warning'">
              {{ inpScore }}
            </div>
          </div>
          <div class="text-4xl font-bold font-mono" :class="inpScore === 'GOOD' ? 'text-success' : inpScore === 'POOR' ? 'text-danger' : 'text-warning'">
...
const totalLatency = computed(() => inputDelay.value + processingTime.value + presentationDelay.value)
...
const handleSlowClick = () => {
  ...
  while(performance.now() - start < 600) { /* block */ }
