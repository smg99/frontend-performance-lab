<template>
  ...
  >
    ...
    >
      <div class="flex-1">
        <h4 class="text-sm font-semibold text-foreground-primary mb-1">Web Worker Pool</h4>
        <p class="text-xs text-foreground-muted">
          Distributes heavy calculations across multiple background threads to maximize multi-core
          CPU utilization.
        </p>
      </div>
      <div class="flex flex-col gap-2">
        <div class="flex gap-2">
          <button
            v-for="n in [1, 2, 4, 8]"
            :key="n"
            class="px-3 py-1 bg-background border rounded text-xs transition-colors"
            :class="
              poolSize === n
                ? 'border-primary text-primary font-bold'
                : 'border-border-subtle text-foreground-muted hover:text-foreground-primary'
            "
            @click="poolSize = n"
          >
            {{ n }} Worker{{ n > 1 ? 's' : '' }}
    ...
    <div v-if="totalTime > 0" class="flex gap-4">
      <div
        class="bg-background-surface border border-border-subtle rounded px-4 py-2 flex flex-col items-center flex-1"
      >
        <span class="text-xs text-foreground-muted uppercase">Total Time</span>
        <span class="text-xl font-bold text-primary">{{ totalTime.toFixed(0) }} ms</span>
      </div>
      <div
        class="bg-background-surface border border-border-subtle rounded px-4 py-2 flex flex-col items-center flex-1"
      >
        <span class="text-xs text-foreground-muted uppercase">Speedup</span>
    ...
    <div class="grid grid-cols-1 gap-4">
      ...
      <div
        ...
        :key="worker.id"
        class="bg-background-surface border border-border-subtle rounded-lg p-4 flex flex-col relative overflow-hidden"
      >
        <div class="flex justify-between items-center mb-2 z-10 relative">
          <h5 class="text-sm font-bold text-foreground-muted flex items-center gap-2">
            Thread #{{ worker.id }}
            <span
              class="w-2 h-2 rounded-full"
              :class="worker.isBusy ? 'bg-warning animate-pulse' : 'bg-success'"
            />
          </h5>
        ...
        <div class="text-[10px] text-foreground-muted text-right z-10 relative">
