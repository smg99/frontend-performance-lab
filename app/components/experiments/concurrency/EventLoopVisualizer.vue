<template>
  ...
  >
    ...
    >
      <div class="flex-1">
        <h4 class="text-sm font-semibold text-foreground-primary mb-1">Event Loop Architecture</h4>
        <p class="text-xs text-foreground-muted">
          A live visualization of the JavaScript Event Loop, Call Stack, and Browser Web APIs.
        </p>
      </div>
      <div class="flex gap-2">
        <button
          class="px-3 py-1.5 bg-text-primary text-background border border-text-primary rounded text-sm hover:bg-text-secondary transition-colors font-bold"
          @click="trigger('sync')"
        >
          Sync Fn
        </button>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 relative">
      ...
      <div class="bg-background-surface border border-border-subtle rounded-lg p-4 flex flex-col">
        <h5
          class="text-sm font-bold text-foreground-primary mb-3 text-center border-b border-border-subtle pb-2"
        >
          Call Stack
        </h5>
        <div class="flex-1 flex flex-col justify-end gap-2 min-h-[200px]">
          <transition-group name="stack">
            <div
              ...
              :key="item.id"
              class="bg-background border border-border-subtle text-foreground-primary text-xs font-mono p-3 rounded text-center shadow-lg"
            >
              {{ item.name }}
            </div>
          </transition-group>
      ...
      <!-- Web APIs -->
      <div
        class="bg-background-surface border border-border-subtle rounded-lg p-4 flex flex-col relative"
      >
        <h5
          class="text-sm font-bold text-foreground-primary mb-3 text-center border-b border-border-subtle pb-2"
        >
      ...
      <div class="flex flex-col gap-4">
        <!-- Microtasks -->
        <div
          class="bg-background-surface border border-border-subtle rounded-lg p-4 flex flex-col flex-1"
        >
          <h5
