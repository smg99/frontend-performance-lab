<template>
  <div class="bg-card border border-border p-6 rounded-xl shadow-subtle flex flex-col gap-6">
    <div class="flex items-center gap-4 bg-surface p-4 rounded-lg border border-border">
      <div class="flex-1">
        <h4 class="text-sm font-semibold text-text-primary mb-1">Microtasks vs Macrotasks</h4>
        <p class="text-xs text-text-secondary">Explore how the JavaScript engine prioritizes Promise microtasks over setTimeout macrotasks.</p>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="bg-surface border border-border rounded-lg p-4 flex flex-col">
        <h5 class="text-sm font-bold text-purple-400 mb-3 text-center border-b border-border pb-2">Microtask Queue</h5>
        <div class="flex-1 flex flex-col justify-end gap-2 min-h-[150px]">
          ...
          <div v-if="microtasks.length === 0" class="text-text-secondary text-xs italic text-center py-4">Empty</div>
        </div>
      </div>

      <!-- Macrotask Queue -->
      <div class="bg-surface border border-border rounded-lg p-4 flex flex-col">
        <h5 class="text-sm font-bold text-blue-400 mb-3 text-center border-b border-border pb-2">Macrotask Queue</h5>
        <div class="flex-1 flex flex-col justify-end gap-2 min-h-[150px]">
          <transition-group name="queue">
            <div 
          ...
          <div v-if="macrotasks.length === 0" class="text-text-secondary text-xs italic text-center py-4">Empty</div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-surface border border-border rounded-lg p-4">
      <h5 class="text-sm font-bold text-text-secondary mb-3">Execution Log</h5>
      <div class="bg-background border border-border rounded p-3 h-32 overflow-y-auto font-mono text-xs space-y-1">
        <div v-for="(log, idx) in logs" :key="idx" :class="log.color">
          > {{ log.message }}
        </div>
        <div v-if="logs.length === 0" class="text-text-secondary italic">Waiting for execution...</div>
      </div>
      <button class="mt-2 text-xs text-text-secondary hover:text-text-primary uppercase font-bold tracking-wider" @click="logs = []">Clear Log</button>
    </div>
  </div>
</template>
...
const flushQueues = async () => {
  ...
  logs.value.push({ message: '--- Event Loop Tick Start ---', color: 'text-text-secondary' })
  ...
  for (const task of currentMicro) {
    ...
    logs.value.push({ message: `Executed Microtask: ${task.name} #${task.id}`, color: 'text-purple-400' })
  }

  if (currentMicro.length > 0) {
    logs.value.push({ message: 'Microtask queue empty. Moving to Macrotasks.', color: 'text-text-secondary' })
  }

  for (const task of currentMacro) {
    await new Promise(resolve => setTimeout(resolve, 500))
    logs.value.push({ message: `Executed Macrotask: ${task.name} #${task.id}`, color: 'text-blue-400' })
  }
