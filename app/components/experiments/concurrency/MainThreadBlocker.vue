  ...
  >
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      >
        ...
        <div
          class="w-16 h-16 rounded-full border-4 border-t-primary border-r-primary border-b-border border-l-border shadow-lg animate-[spin_1s_linear_infinite]"
        />
        <p class="text-xs text-foreground-muted mt-6 text-center">
          CSS animations might keep running (if composited) but usually jank when the main thread
          blocks.
        </p>
      </div>
    ...
    >
      <div class="flex justify-between items-center">
        <div>
          <h4 class="font-bold text-foreground-primary">Synchronous Workload</h4>
          <p class="text-xs text-foreground-muted">Runs directly on the Main Thread</p>
        </div>
        <button
          :disabled="isRunning"
      ...
      >
        <div class="flex justify-between">
          <span class="text-foreground-muted">Execution Time (Blocked):</span>
