  ...
  <div class="bg-card border border-border p-6 rounded-xl shadow-subtle flex flex-col gap-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div class="bg-surface border border-border rounded-lg p-6 flex flex-col items-center">
        ...
        <div class="w-16 h-16 rounded-full border-4 border-t-info border-r-info border-b-border border-l-border shadow-lg animate-[spin_1s_linear_infinite]"/>
        <p class="text-xs text-text-secondary mt-6 text-center">Spins perfectly smoothly because the Main Thread is not blocked.</p>
      </div>
    ...
    <div class="flex flex-col gap-4 bg-surface p-4 rounded-lg border border-border">
      <div class="flex justify-between items-center">
        <div>
          <h4 class="font-bold text-text-primary">Asynchronous Workload (Web Worker)</h4>
          <p class="text-xs text-text-secondary">Offloaded to a background thread</p>
        </div>
        <button 
          :disabled="isRunning"
          ...
          @click="runHeavyTask"
        >
          <span v-if="isRunning" class="w-4 h-4 rounded-full border-2 border-white border-t-transparent animate-spin"/>
      ...
      <div v-if="lastResult" class="bg-background border border-border rounded p-3 text-sm font-mono flex flex-col gap-1">
        <div class="flex justify-between">
          <span class="text-text-secondary">Execution Time (Background):</span>
