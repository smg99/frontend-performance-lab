  ...
  >
    ...
    >
      <div class="flex-1">
        ...
        <p class="text-xs text-foreground-muted">
          Compare how the browser schedules visual updates (rAF) vs background low-priority work
          (rIC).
        </p>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="bg-background-surface border border-border-subtle rounded-lg p-4 flex flex-col">
        ...
        <div class="text-xs text-foreground-muted mb-4 h-12">
          Schedules work to be executed right before the next repaint. Runs smoothly with the
          display refresh rate (typically 60 times/sec).
        </div>
        <div
          class="w-full h-12 bg-background border border-border-subtle relative overflow-hidden rounded"
        >
          <div
        ...
        <div class="mt-4 font-mono text-xs text-foreground-muted flex justify-between">
          <span
            >Frames: <strong class="text-foreground-primary">{{ rafFrames }}</strong></span
          >
        </div>
      </div>
      ...
      <div class="bg-background-surface border border-border-subtle rounded-lg p-4 flex flex-col">
        ...
        <div class="text-xs text-foreground-muted mb-4 h-12">
          Schedules work to execute when the main thread is completely idle. Drops significantly
          when you click "Block Thread".
        </div>
        <div
          class="w-full h-12 bg-background border border-border-subtle relative overflow-hidden rounded"
        >
        ...
        <div class="mt-4 font-mono text-xs text-foreground-muted flex justify-between">
          <span
            >Callbacks: <strong class="text-foreground-primary">{{ ricFrames }}</strong></span
          >
          <span
            >Time Remaining (last):
            <strong class="text-foreground-primary"
              >{{ lastRicTimeRemaining.toFixed(1) }}ms</strong
            ></span
          >
        </div>
      </div>
...
const startRIC = () => {
  ...
  const updateRIC = (deadline: IdleDeadline) => {
    ...
    if ('requestIdleCallback' in window) {
      ...
      ricId = setTimeout(
        () => updateRIC({ timeRemaining: () => 50, didTimeout: false }),
        50
