<template>
  <section class="py-24 max-w-7xl mx-auto px-4 border-b border-border-subtle-subtle">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-x-6 gap-y-10">
      ...
      >
        ...
        >
          <template #badges>
            <Badge variant="outline" class="text-xs text-primary border-primary/20 bg-primary/5"
              >Recipe</Badge
            >
          </template>
        </FeatureCard>
      ...
      >
        ...
        >
          <template #badges>
            <Badge variant="outline" class="text-xs text-blue-500 border-blue-500/20 bg-blue-500/5"
              >Native API</Badge
            >
          </template>
        </FeatureCard>
...
const featuredExperiment = computed(
  () => getAllExperiments().find(e => e.featured) || getAllExperiments()[0]
)
const featuredRecipe = computed(() => getAllRecipes().find(r => r.featured) || getAllRecipes()[0])
const featuredApi = computed(
  () => getAllBrowserAPIs().find(a => a.featured) || getAllBrowserAPIs()[0]
)
</script>
