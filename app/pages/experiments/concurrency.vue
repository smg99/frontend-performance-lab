<template>
  <div class="space-y-10 pb-10">
    <div class="border-b border-border pb-6">
      <h2 class="text-4xl font-bold text-text-primary tracking-tight">Event Loop & Concurrency</h2>
      <p class="text-text-secondary mt-2 text-xl">An interactive engineering playground for understanding JavaScript's single thread, jank, and Web Workers.</p>
    </div>
    ...
    <section>
      ...
      <p class="text-text-secondary mb-4">
        JavaScript runs on a single main thread. The Event Loop is responsible for executing code, collecting and processing events, and executing queued sub-tasks.
      </p>
      <EventLoopVisualizer />
    </section>
    ...
    <section>
      ...
      <p class="text-text-secondary mb-4">
        If you execute heavy, synchronous work, the entire browser tab freezes. Animations stop, clicks are ignored, and FPS drops to 0. 
        Try starting the animation below, then block the main thread.
      </p>
      <MainThreadBlocker />
    </section>
    ...
    <section>
      ...
      <p class="text-text-secondary mb-4">
        Web Workers run on separate background threads. By offloading heavy work (like sorting or hashing), the main thread remains free to render animations at a buttery smooth 60 FPS.
      </p>
      <WebWorkerDemo />
    </section>
    ...
    <section>
      ...
      <p class="text-text-secondary mb-4">
        The Event Loop doesn't just execute code top-to-bottom. It maintains separate queues for tasks. 
        <strong>Microtasks</strong> (Promises, <code>queueMicrotask</code>) are given extreme priority and will ALWAYS execute before <strong>Macrotasks</strong> (<code>setTimeout</code>, <code>setInterval</code>, DOM events).
      </p>
      <MicroMacroTasks />
    </section>
    ...
    <section>
      ...
      <p class="text-text-secondary mb-4">
        Web Workers are heavy. Instead of creating a new worker for every small task, a Worker Pool allocates tasks to a fixed number of workers.
      </p>
      <WorkerPool />
    </section>
    ...
    <section>
      ...
      <p class="text-text-secondary mb-4">
        Compare how visual updates (rAF) sync with the display refresh rate, while background tasks (rIC) only execute during idle time.
      </p>
      <RAFvsIdle />
    </section>
    ...
    <section>
      ...
      <p class="text-text-secondary mb-4">
        A visual breakdown of how the browser schedules the rendering pipeline across threads during a typical frame.
      </p>
      <Timeline />
    </section>

    <!-- Educational Framework Integration -->
    <LearningSummaryCard :data="learningData" class="mx-auto" />
  </div>
</template>
...
const learningData: LearningSummaryData = {
  ...
  whatIsIt: 'JavaScript is strictly single-threaded, meaning it can only execute one piece of code at a time. The Event Loop is the mechanism that allows JS to perform non-blocking I/O operations (like fetching data or timeouts) despite being single-threaded.',
  howItWorks: 'The browser places synchronous code on the Call Stack. When asynchronous code is encountered, it is handed off to browser Web APIs. Once complete, callbacks are pushed to the Macrotask Queue (setTimeout) or Microtask Queue (Promises). The Event Loop constantly checks: if the Call Stack is empty, it pushes tasks from the queues back onto the stack to run.',
  recommendation: {
    approach: 'Offload heavy synchronous calculations to Web Workers.',
    reasoning: 'Any JavaScript execution that takes longer than 50ms (a "Long Task") blocks the browser from updating the UI, processing clicks, or scrolling. Web Workers run on a separate OS thread, completely freeing the Main Thread.',
...
self.onmessage = (e) => {
  ...
  decisionMatrix: [
    { situation: 'Fetching API data', recommended: 'async/await (Microtask)', alternative: 'Promises', avoid: 'Synchronous XHR' },
    { situation: 'Heavy JSON parsing / Crypto', recommended: 'Web Worker', alternative: 'requestIdleCallback chunking', avoid: 'Main thread processing' },
    { situation: 'DOM Animation / Visual updates', recommended: 'requestAnimationFrame', alternative: 'CSS Animations', avoid: 'setTimeout' },
    { situation: 'Non-critical analytics ping', recommended: 'requestIdleCallback', alternative: 'setTimeout', avoid: 'Blocking main execution' }
  ],
  commonMistakes: [
    { 
      problem: 'Blocking the main thread with heavy math.', 
      impact: 'The entire browser tab freezes. Gifs stop spinning, buttons cannot be clicked, and scrolling stutters.', 
...
setInterval(() => {
  ...
  interviewQuestions: [
    { question: 'What is the difference between a Microtask and a Macrotask?', difficulty: 'Intermediate', answer: 'Microtasks (Promises, MutationObserver) have higher priority. After a Macrotask (setTimeout, DOM event) finishes, the Event Loop will empty the ENTIRE Microtask queue before rendering the DOM or moving to the next Macrotask.' },
    { question: 'If you have an infinite loop in a Promise.then() block, what happens?', difficulty: 'Advanced', answer: 'The browser tab will freeze and eventually crash. Because Promise.then() creates Microtasks, and the Event Loop processes ALL Microtasks before rendering, the rendering phase will never be reached.' }
  ],
  proTips: [
