<template>
  <div class="space-y-6 flex flex-col h-full">
    <div>
      <h2 class="text-3xl font-bold text-text-primary tracking-tight">Rendering Virtualization</h2>
      <p class="text-text-secondary mt-1 text-lg">Compare different strategies for rendering massive lists.</p>
    </div>

    <!-- Controls -->
    <div class="bg-card border border-border rounded-xl p-4 shadow-subtle flex flex-col sm:flex-row gap-4 justify-between items-center">
      <div class="flex space-x-2 bg-surface p-1 rounded-lg border border-border">
        <button 
          ...
          :class="[
            ...
            activeStage === stage.id ? 'bg-primary text-white shadow' : 'text-text-secondary hover:text-text-primary hover:bg-border/50'
          ]"
      ...
      <div class="flex items-center space-x-3">
        ...
        <select v-model="itemCount" class="bg-surface border border-border text-text-primary text-sm rounded-lg focus:ring-primary focus:border-primary block p-2 outline-none">
    ...
    <!-- Implementation Area -->
    <div class="flex-1 min-h-[500px] bg-card border border-border rounded-xl shadow-subtle overflow-hidden flex flex-col">
...
const stages = [
  { id: 1, name: 'Stage 1: Naive', description: 'Renders every single DOM node. Watch your memory and FPS tank on large lists.' },
  { id: 2, name: 'Stage 2: VueUse', description: 'Uses the @vueuse/core useVirtualList composable for basic virtualization.' },
  { id: 3, name: 'Stage 3: Custom', description: 'A fully custom virtualization engine demonstrating buffer management, recycling, and positioning.' },
]
...
const learningData: LearningSummaryData = {
  ...
  whatIsIt: 'List virtualization (or windowing) is the technique of rendering only the items that are currently visible within the scrolling viewport, rather than rendering the entire dataset to the DOM at once.',
  howItWorks: 'Instead of creating 100,000 DOM nodes, a virtual list calculates the height of all items, creates a massive scrolling container, and absolutely positions a tiny subset of nodes (e.g. 20) inside the visible area. As you scroll, those same 20 nodes are constantly repositioned and recycled with new data.',
  recommendation: {
...
const { list, containerProps, wrapperProps } = useVirtualList(
  ...
  decisionMatrix: [
    { situation: '< 500 items', recommended: 'Standard v-for', alternative: 'Pagination', avoid: 'Virtualization (Overkill)' },
    { situation: '1,000+ items (Fixed Height)', recommended: 'useVirtualList (@vueuse)', alternative: 'Custom Fixed Virtualizer', avoid: 'Standard v-for' },
    { situation: '10,000+ items (Variable Height)', recommended: 'vue-virtual-scroller', alternative: 'TanStack Virtual', avoid: 'Standard v-for' }
  ],
  commonMistakes: [
    { 
  ...
  <div v-bind="containerProps">
    ...
    { problem: 'Virtualizing small lists.', impact: 'Unnecessary calculation overhead for simple lists.', fix: 'Only virtualize if DOM node count exceeds ~1500.' }
  ],
  interviewQuestions: [
    { question: 'What causes the performance drop when rendering 100,000 items?', difficulty: 'Intermediate', answer: 'The browser has to allocate memory for 100k DOM objects, parse their CSS rules, and calculate Layout for all of them synchronously. The JS heap balloons, and the main thread blocks for several seconds.' },
    { question: 'How do virtual lists handle scrolling without losing the scrollbar size?', difficulty: 'Advanced', answer: 'They calculate the total theoretical height of all items (e.g. 100k * 50px = 5M px) and place a hidden spacer div of that height inside the container. This forces the browser to draw an accurate scrollbar.' }
  ],
  proTips: [
